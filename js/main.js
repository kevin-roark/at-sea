...
(function atsea() {
  ...
  function splash() {
    var splashImages = document.querySelectorAll('.splash-image');

    var accumulatedDelay = 1000;
    for (var i = splashImages.length - 1; i >= 0; i--) {
      scale(i, accumulatedDelay);
      accumulatedDelay += (4500 / splashImages.length - 1 - i);
    }

    setTimeout(seaMe, accumulatedDelay + 1000);

    function seaMe() {
      var splashEl = document.body.querySelector('.splash');
    ...
    function scale(index, delay) {
      setTimeout(function() {
        if (index > 1) {
          splashImages[index - 2].classList.add('shadow-splash-image');
        }

        var splashImage = splashImages[index];
        var scale = (splashImages.length - index) * 0.005;
        splashImage.style.webkitTransform = splashImage.style.mozTransform = splashImage.style.transform = 'scale(' + scale + ')';
      }, delay);
    }
  }
